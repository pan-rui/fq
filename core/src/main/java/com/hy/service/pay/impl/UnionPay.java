 ...
 */
@Service
public class UnionPay implements IPaymentService {
    private Logger logger = LogManager.getLogger(UnionPay.class);
    @Value("#{config['MERID']}")
...
//        signType=bundle.getString("signType");
    ...
    public BaseResult recharge(Map<String, Object> param) {
        ParamsMap paramsMap = ParamsMap.newMap("version", SDKConfig.getConfig().getVersion()).addParams("encoding", IBase.DEF_CHATSET).addParams("signMethod", SDKConfig.getConfig().getSignMethod())
                .addParams("txnType", "11").addParams("txnSubType", "02").addParams("bizType", "000501").addParams("channelType", channelType).addParams("merId", merId).addParams("accessType", accessType)
                ...
                .addParams("encryptCertId", AcpService.getEncryptCertId()).addParams("accNo", AcpService.encryptData((String) param.get("accNo"), IBase.DEF_CHATSET)).addParams("backUrl", SDKConfig.getConfig().getBackUrl())
                .addParams(param.get("customKey"), param.get("customVal"));
...
/*                else if(("03").equals(respCode)||
            ...
            //未返回正确的http状态
            logger.error("====================银联代收出现网络故障...."+rspData);
        }
