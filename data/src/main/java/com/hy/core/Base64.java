...
public final class Base64 {
    ...
    public static String md5(String username,String password) throws NoSuchAlgorithmException {
        try {
            return DigestUtils.md5Hex((username+password).getBytes("utf-8"));
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
            throw new RuntimeException(e);
        }
    ...
    public static <T> T[] mergeArray(T[] src, T[] src2) {
        Class type=null;
        if(src.length>0)
            type=src[0].getClass();
        if(type==null&&src2.length>0)
            type=src2[0].getClass();
        else throw new ArrayIndexOutOfBoundsException();
        T[] newB = (T[]) Array.newInstance(type,src.length + src2.length);
